...
(function (exports) {
  exports.LevelGraph = React.createClass({
    ...
    renderGraph: function renderGraph() {
      ...
      var yScale = d3.scale.linear().domain([Ymin, Ymax]).range([padding, h - padding]);
      //類似X軸的尺度
      var yScale = d3.scale.linear().domain([Ymin, Ymax]).range([padding, h - padding]);
      //類似X軸的尺度

      var colorScale = d3.scale.linear().domain([Ymin, Ymax]).range([0, 700]);
      //這次顏色都要用尺度來算
